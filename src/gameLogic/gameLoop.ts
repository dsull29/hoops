...
export const processTurn = (
  ...
  if (!nextEvent) {
    ...
    if (Math.random() < 0.1 && nextPlayerState.stats.energy < 30 && !nextEvent) {
      ...
      (nextPlayerState.totalDaysPlayed % 2 === 1 || nextPlayerState.totalDaysPlayed % 5 === 1) &&
      Math.random() < 0.4 &&
      !nextEvent
    ) {
      // Using the function from contextualEventRunner.ts
      const contextualEvent = getModeRoleContextualEvent(nextPlayerState);
      if (contextualEvent) {
        nextEvent = contextualEvent;
        nextPlayerState.careerLog = [
          ...nextPlayerState.careerLog,
          `--- Contextual Event: ${contextualEvent.title} ---`,
        ];
        eventTriggerMessage =
          (eventTriggerMessage ? eventTriggerMessage + ' & ' : '') +
          `Story beat: ${contextualEvent.title}!`;
      }
    }
