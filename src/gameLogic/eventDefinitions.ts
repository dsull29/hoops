...
import type { Player, GameEvent, Choice, PlayerStats } from "../types";
...
export const createDailyChoiceEvent = (player: Player): GameEvent => {
  const choices: Choice[] = [
    {
      ...
      action: (p) => {
        ...
        const gain =
          Math.random() < newStats.professionalism / MAX_STAT_VALUE ? 2 : 1;
        newStats.shooting = clamp(
          newStats.shooting + gain,
          MIN_STAT_VALUE,
          MAX_STAT_VALUE
        );
        ...
        return {
          ...
          outcomeMessage: `Focused on shooting. ${
            gain > 1 ? "Felt a real improvement!" : "Good session."
          } Shooting +${gain}, Energy -20.`,
        };
      },
    ...
    },
    {
      ...
      action: (p) => {
        ...
        return {
          updatedPlayer: { ...p, stats: newStats },
          outcomeMessage: `Pushed hard on athleticism. ${
            gain > 1 ? "Feeling stronger!" : "Solid workout."
          } Athleticism +${gain}, Energy -25.`,
        };
      },
    ...
    },
    {
      ...
      action: (p) => {
        ...
        return {
          updatedPlayer: { ...p, stats: newStats },
          outcomeMessage: `Spent time studying film. ${
            gain > 1 ? "Key insights gained!" : "Learned a few things."
          } BBIQ +${gain}, Energy -10.`,
        };
      },
    ...
    },
    {
      id: "rest",
      text: "Rest & Recover",
      description: "Take a day off to recover energy and morale.",
      action: (p) => {
        const newStats = { ...p.stats };
        ...
        newStats.morale = clamp(
          newStats.morale + 5 + Math.floor(Math.random() * 6),
          0,
          MAX_MORALE
        );
        return {
          updatedPlayer: { ...p, stats: newStats },
          outcomeMessage:
            "Took a much-needed rest day. Feeling refreshed. Energy and Morale increased.",
        };
      },
    },
    {
      ...
      description: "Hang out with teammates or attend a public function.",
      cost: { stat: "energy", amount: 15 },
      action: (p) => {
        const newStats = { ...p.stats };
        newStats.energy = clamp(newStats.energy - 15, 0, MAX_ENERGY);
        let outcomeMessage = "Attended a social event. ";
        ...
        if (charismaRoll < 0.3) {
          ...
          outcomeMessage += "It was a bit awkward. Charisma -2, Morale -5.";
        } else if (charismaRoll < 0.7) {
          newStats.charisma = clamp(
            newStats.charisma + 1,
            MIN_STAT_VALUE,
            MAX_STAT_VALUE
          );
          outcomeMessage += "Made a few connections. Charisma +1.";
        } else {
          newStats.charisma = clamp(
            newStats.charisma + 3,
            MIN_STAT_VALUE,
          ...
          outcomeMessage += "It was a great time! Charisma +3, Morale +10.";
        }
  ...
  return {
    id: "daily_choices",
    title: `Season ${player.currentSeason}, Day ${player.currentDayInSeason} (${player.age} y.o.)`,
...
export const gameDayEvent = (player: Player): GameEvent => {
  return {
    id: "game_day",
    title: `Game Day! (Season ${player.currentSeason}, Day ${player.currentDayInSeason})`,
    description: "It's time to hit the court. How will you approach this game?",
    isMandatory: true,
    choices: [
      {
        id: "play_hard",
        text: "Play Hard (High Risk/Reward)",
        cost: { stat: "energy", amount: 30 },
        action: (p) => {
          const newStats = { ...p.stats };
          newStats.energy = clamp(
            newStats.energy - 30 - Math.floor(Math.random() * 10),
            0,
            MAX_ENERGY
          );
          let outcomeMessage = "You played hard! ";
          const performanceScore =
            (newStats.shooting + newStats.athleticism + newStats.basketballIQ) /
              3 +
            (Math.random() * 20 - 10);
          if (performanceScore > 65) {
            newStats.morale = clamp(newStats.morale + 15, 0, MAX_MORALE);
            const statBoost = ["shooting", "athleticism", "basketballIQ"][
              Math.floor(Math.random() * 3)
            ] as keyof PlayerStats;
            newStats[statBoost] = clamp(
              (newStats[statBoost] as number) + 1,
              MIN_STAT_VALUE,
              MAX_STAT_VALUE
            );
            outcomeMessage += `Stellar performance! You led the team. Morale +15, ${statBoost} +1.`;
          } else if (performanceScore > 45) {
            newStats.morale = clamp(newStats.morale + 5, 0, MAX_MORALE);
            outcomeMessage += "Solid contribution to the team. Morale +5.";
          } else {
            newStats.morale = clamp(newStats.morale - 10, 0, MAX_MORALE);
            outcomeMessage +=
              "Tough game, couldn't find your rhythm. Morale -10.";
          }
          return { updatedPlayer: { ...p, stats: newStats }, outcomeMessage };
        },
      ...
      {
        id: "play_cautiously",
        text: "Play Cautiously (Low Risk/Reward)",
        cost: { stat: "energy", amount: 20 },
        action: (p) => {
          const newStats = { ...p.stats };
          newStats.energy = clamp(newStats.energy - 20, 0, MAX_ENERGY);
          let outcomeMessage = "You played cautiously. ";
          const performanceScore =
            (newStats.shooting + newStats.athleticism + newStats.basketballIQ) /
              3 +
            (Math.random() * 10 - 5);
          if (performanceScore > 55) {
            newStats.morale = clamp(newStats.morale + 5, 0, MAX_MORALE);
            outcomeMessage += "Played your role effectively. Morale +5.";
          } else {
            newStats.morale = clamp(newStats.morale - 5, 0, MAX_MORALE);
            outcomeMessage +=
              "Quiet game, didn't make much of an impact. Morale -5.";
          }
          return { updatedPlayer: { ...p, stats: newStats }, outcomeMessage };
        },
        disabled: (p) => p.stats.energy < 20,
...
export const agentMeetingEvent = (player: Player): GameEvent => {
  return {
    ...
    description: `Your agent, ${
      player.traits.includes("Good Agent")
        ? "the reliable Sarah Chen,"
        : 'the somewhat shady Vinny "The Shark" Gambino,'
    } wants to discuss your future.`,
    isMandatory: true,
    choices: [
      {
        id: "discuss_contract",
        ...
        action: (p) => {
          ...
          if (p.stats.charisma > 60 || p.traits.includes("Good Agent")) {
            outcomeMessage +=
              "Your agent seems confident they can get you a good deal soon.";
            p.stats.morale = clamp(p.stats.morale + 5, 0, MAX_MORALE);
          } else {
            outcomeMessage +=
              "The conversation was a bit tense. No clear path forward yet.";
      ...
      },
      {
        ...
        action: (p) => {
          ...
          if (
            p.stats.charisma > 70 &&
            p.stats.shooting + p.stats.athleticism > 120
          ) {
            outcomeMessage +=
              "A local shoe store offers you a small deal! Morale +10.";
            p.stats.morale = clamp(p.stats.morale + 10, 0, MAX_MORALE);
          } else if (p.traits.includes("Good Agent")) {
            outcomeMessage +=
              "Sarah thinks she can find something if you keep performing well.";
          } else {
            outcomeMessage +=
              "Vinny says 'Leave it to me, kid!' but you're not so sure.";
          }
          return { updatedPlayer: p, outcomeMessage };
        },
      },
      {
        ...
        action: (p) => {
          return {
            updatedPlayer: p,
            outcomeMessage:
              "You told your agent you're focused on the game right now.",
          };
        },
      },
    ],
