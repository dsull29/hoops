...
export const createDailyChoiceEvent = (player: Player): GameEvent => {
  const choices: Choice[] = [
    {
      ...
      action: (p) => {
        ...
        const gain = Math.random() < newStats.professionalism / MAX_STAT_VALUE ? 2 : 1;
    ...
    },
    {
      ...
      action: (p) => {
        const newStats = { ...p.stats };
        const gain = Math.random() < newStats.professionalism / MAX_STAT_VALUE ? 2 : 1;
        newStats.basketballIQ = clamp(newStats.basketballIQ + gain, MIN_STAT_VALUE, MAX_STAT_VALUE);
    ...
    },
    {
      ...
      action: (p) => {
        ...
        if (charismaRoll < 0.3) {
          ...
          newStats.charisma = clamp(newStats.charisma + 3, MIN_STAT_VALUE, MAX_STAT_VALUE);
  ...
  return {
    id: 'daily_choices',
    title: `${player.gameMode} (${player.currentRole}) - Season ${player.currentSeasonInMode}, Day ${player.currentDayInSeason} (${player.age} y.o.)`,
...
export const gameDayEvent = (player: Player): GameEvent => {
  return {
    id: 'game_day',
    title: `Game Day! (${player.gameMode} - ${player.currentRole})`,
    ...
    choices: [
      {
        ...
        action: (p) => {
          ...
          const performanceScore =
            (newStats.shooting + newStats.athleticism + newStats.basketballIQ) / 3 +
            (Math.random() * 20 - 10);
          if (performanceScore > 65) {
            ...
            newStats.morale = clamp(newStats.morale - 10, 0, MAX_MORALE);
            outcomeMessage += "Tough game, couldn't find your rhythm. Morale -10.";
          }
          return { updatedPlayer: { ...p, stats: newStats }, outcomeMessage };
        },
      ...
      },
      {
        ...
        action: (p) => {
          ...
          if (performanceScore > 55) {
            ...
            newStats.morale = clamp(newStats.morale - 5, 0, MAX_MORALE);
            outcomeMessage += "Quiet game, didn't make much of an impact. Morale -5.";
          }
          return { updatedPlayer: { ...p, stats: newStats }, outcomeMessage };
        },
        disabled: (p) => p.stats.energy < 20,
